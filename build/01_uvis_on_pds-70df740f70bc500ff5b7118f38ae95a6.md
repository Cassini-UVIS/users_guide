...
```{image} figures/fig_1.0.png
:alt: Picture of UVIS instrument
:width: 60%
```
...
More descriptions on the handling of RTG background can be found in Chapters {ref}`sec:calib`, {ref}`sec:saturns_aurora`, {ref}`sec:rings_spec`, and {ref}`sec:icy_moons`.
...
Users can find more information regarding backgrounds, the flatfield correction, and calibration in Chapters {ref}`sec:calib` and {ref}`sec:iph_survey`.
...
Users can find how the “evil” pixels are identified and handled in Chapters {ref}`sec:calib` and {ref}`sec:saturns_aurora`. 
An alternate approach is to average over large enough areas of the detector to minimize the effects. 
This is described in {ref}`sec:stellar_calib_reduc_method`.
...
:::{figure} figures/fig_1.1.*
:label: "fig:saturn-aurora"
:alt: Saturn's aurora on 2005-172
...
:::{figure} figures/fig_1.2.*
:label: "fig:phoebe-dayside"
:alt: Phoebe's dayside
...
:::{figure} figures/fig_1.3.*
:label: "fig:enc-occ"
:alt: 2005 Enceladus occultation
